...
class EventDrivenEngine:
    def __init__(
        ...
        max_sector_overweight=0.40,  # Max overweight vs S&P 500 in any sector
        trailing_stop=0,      # Sell if price drops 25% from peak (0 = disabled)
        weight_by_score=False,    # Weight positions by score (False = equal weight)
        ...
        self.max_sector_overweight = max_sector_overweight
    ...
    def run(self, quiet=False):
        ...
        if self.max_sector_overweight < 1.0:
            try:
                ...
                if not quiet:
                    print(f"  Loaded S&P 500 market cap data for sector caps")
            except Exception as e:
                if not quiet:
                    print(f"  Warning: could not load S&P market caps: {e}")

        # Cache for S&P sector weights (computed monthly)
        sp_sector_weight_cache = {}
        ...
        for day in trading_days:
            ...
            for ticker in tickers_to_evaluate:
                ...
                if ticker in positions:
                    ...
                    if not passes_screen:
                        ...
                        if score >= self.buy_threshold and passes_screen:
                            # Renew the position (keep peak price)
                ...
                # === BUY LOGIC ===
                    ...
                    # Check correlation — if too correlated, consider swapping
                    corr_conflict = self._find_correlated_holding(ticker, positions, price_matrix, day)
                    if corr_conflict is not None:
                        # New ticker is correlated with corr_conflict — keep the higher scorer
                        old_pos = positions[corr_conflict]
                        if score > old_pos.score:
                            # Swap: sell the old, buy the new
                        ...
                        # else: old holding has higher score, keep it
                        continue

                    # No correlation conflict — check if we have room
            ...
            if self.trailing_stop > 0:
                for ticker in list(positions.keys()):
                    ...
                    if days_held >= self.min_hold_days:
                        stop_price = pos.peak_price * (1 - self.trailing_stop)
                    ...
                    for t in held_tickers:
                        ...
                        if pd.notna(p0) and pd.notna(p1) and p0 > 0:
                            ...
                            if abs(r) < 1.0:  # filter extreme
                                day_rets.append(r)
                                day_weights.append(positions[t].score if self.weight_by_score else 1.0)
                    if day_rets:
                        weights = np.array(day_weights)
                        weights = np.maximum(weights, 1.0)  # floor at 1 to avoid negative weights
                        weights = weights / weights.sum()
                        port_ret = np.dot(day_rets, weights)
                    else:
                        port_ret = 0.0
                else:
                    port_ret = 0.0
        ...
        if trades:
            ...
            if not quiet:
                n_stops = sum(1 for t in trades if t.reason == "trailing_stop")
                print(f"\n  Total trades: {len(trades)} ({n_stops} trailing stops)")
    ...
    def _get_sp_sector_weights(self, day, sp_mcap_data, sector_map, cache):
        """Get S&P 500 sector weights for a given date (cached monthly)."""
        month_key = (day.year, day.month)
        if month_key in cache:
            return cache[month_key]

        if sp_mcap_data is None:
            return {}

        window = sp_mcap_data[
            (sp_mcap_data["Date"] >= day - pd.Timedelta(days=10)) &
            (sp_mcap_data["Date"] <= day + pd.Timedelta(days=5))
        ]
        if window.empty:
            cache[month_key] = {}
            return {}
        ...
        if total > 0:
            weights = (sector_caps / total).to_dict()
        else:
            weights = {}
        cache[month_key] = weights
        return weights

    def _check_sector_cap(self, ticker, positions, sector_map, day, sp_mcap_data, cache):
        """Check if adding ticker would breach sector overweight limit.
        Returns True if OK to buy, False if sector is too overweight.
        """
        if self.max_sector_overweight >= 1.0:
            return True
        ...
        sp_weights = self._get_sp_sector_weights(day, sp_mcap_data, sector_map, cache)
        sp_weight = sp_weights.get(sector, 0.0)

        # Project sector weight assuming equal-weight portfolio
        # Use max(current positions, 5) as denominator to avoid blocking
        # early buys when portfolio is still building up
        same_sector = sum(1 for t in positions if sector_map.get(t, "Unknown") == sector) + 1
        n_positions = max(len(positions) + 1, 5)
        port_weight = same_sector / n_positions

        overweight = port_weight - sp_weight
        return overweight <= self.max_sector_overweight

    def _find_correlated_holding(self, ticker, positions, price_matrix, day):
        """Find the first holding that is too correlated with ticker.

        Returns the ticker of the conflicting holding, or None if no conflict.
        """
    ...
    def _check_correlation(self, ticker, positions, price_matrix, day):
        ...
        """
        return self._find_correlated_holding(ticker, positions, price_matrix, day) is None
...
def sweep_thresholds():
    """Run parameter sweep with correlation swaps and sector cap active."""
    from itertools import product
    import time as _time

    buy_thresholds = [35, 40, 45, 50, 55]
    sell_thresholds = [15, 20, 25, 30]
    max_positions_list = [12, 15, 20]
    max_hold_list = [180, 270, 365, 540, 720]
    min_hold_list = [30, 60, 90]

    # Fixed: our established features
    ...
    combos = [
        (buy_t, sell_t, max_pos, max_hold, min_hold)
        for buy_t, sell_t, max_pos, max_hold, min_hold
        in product(buy_thresholds, sell_thresholds, max_positions_list, max_hold_list, min_hold_list)
        if sell_t < buy_t and min_hold < max_hold
    ]

    print(f"Sweeping {len(combos)} parameter combinations")
    print(f"Fixed: correlation={max_correlation}, sector_cap={max_sector_overweight}")
    print(f"Params: buy={buy_thresholds}, sell={sell_thresholds}")
    print(f"        positions={max_positions_list}, max_hold={max_hold_list}, min_hold={min_hold_list}")
    print()

    results = []
    t0 = _time.time()

    for i, (buy_t, sell_t, max_pos, max_hold, min_hold) in enumerate(combos):
        elapsed = _time.time() - t0
        eta = (elapsed / max(i, 1)) * (len(combos) - i)
        print(f"[{i+1}/{len(combos)}] buy={buy_t} sell={sell_t} pos={max_pos} "
              f"hold={min_hold}-{max_hold}d  (ETA: {eta/60:.0f}m)", end=" ")
        ...
        engine = EventDrivenEngine(
            ...
            max_sector_overweight=max_sector_overweight,
        )
        try:
            ...
            print(f"=> CAGR={r['cagr']:.2%} Sharpe={r['sharpe']:.2f} "
                  ...
                  f"Trades={n_trades} Swaps={n_swaps}")
            results.append({
                "buy_threshold": buy_t,
                "sell_threshold": sell_t,
                "max_positions": max_pos,
    ...
    print("\n\n" + "=" * 80)
    print("TOP 15 PARAMETER COMBINATIONS (by Sortino + drawdown penalty)")
    print("=" * 80)
    print(f"  {'buy':>3} {'sell':>4} {'pos':>3} {'hold':>9} | "
          f"{'CAGR':>6} {'Sharpe':>6} {'Sortino':>7} {'MaxDD':>7} {'Excess':>6} | "
          f"{'Trades':>6} {'Swaps':>5} {'WinR':>5} {'AvgHold':>7}")
    print("-" * 100)
    for _, row in df.head(15).iterrows():
        print(f"  {row['buy_threshold']:3.0f} {row['sell_threshold']:4.0f} {row['max_positions']:3.0f} "
              f"{row['min_hold_days']:3.0f}-{row['max_hold_days']:3.0f}d | "
              f"{row['cagr']:6.2%} {row['sharpe']:6.2f} {row['sortino']:7.2f} "
              f"{row['max_drawdown']:7.2%} {row['excess_return']:6.2%} | "
              f"{row['n_trades']:6.0f} {row['n_swaps']:5.0f} {row['win_rate']:5.0%} "
              f"{row['avg_hold_days']:5.0f}d")

    # Also show top 15 by CAGR
    ...
    print(f"  {'buy':>3} {'sell':>4} {'pos':>3} {'hold':>9} | "
          ...
          f"{'Trades':>6} {'Swaps':>5} {'WinR':>5} {'AvgHold':>7}")
    print("-" * 100)
    for _, row in df_cagr.head(15).iterrows():
        print(f"  {row['buy_threshold']:3.0f} {row['sell_threshold']:4.0f} {row['max_positions']:3.0f} "
              f"{row['min_hold_days']:3.0f}-{row['max_hold_days']:3.0f}d | "
              f"{row['cagr']:6.2%} {row['sharpe']:6.2f} {row['sortino']:7.2f} "
              f"{row['max_drawdown']:7.2%} {row['excess_return']:6.2%} | "
    ...
    print(f"  {'buy':>3} {'sell':>4} {'pos':>3} {'hold':>9} | "
          ...
          f"{'Trades':>6} {'Swaps':>5} {'WinR':>5} {'AvgHold':>7}")
    print("-" * 100)
    for _, row in df_sharpe.head(15).iterrows():
        print(f"  {row['buy_threshold']:3.0f} {row['sell_threshold']:4.0f} {row['max_positions']:3.0f} "
              f"{row['min_hold_days']:3.0f}-{row['max_hold_days']:3.0f}d | "
              f"{row['cagr']:6.2%} {row['sharpe']:6.2f} {row['sortino']:7.2f} "
              f"{row['max_drawdown']:7.2%} {row['excess_return']:6.2%} | "
    ...
    best = df.iloc[0]
    print(f"Best (composite): buy={best['buy_threshold']:.0f} sell={best['sell_threshold']:.0f} "
          f"pos={best['max_positions']:.0f} hold={best['min_hold_days']:.0f}-{best['max_hold_days']:.0f}d")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-sector-overweight", type=float, default=0.30,
                        help="Max sector overweight vs S&P 500 (0-1)")
    parser.add_argument("--trailing-stop", type=float, default=0.25,
                        help="Trailing stop loss pct (0 to disable)")
    parser.add_argument("--no-score-weight", action="store_true",
                        help="Use equal weight instead of score-weighted")
    ...
    if args.sweep:
        ...
        engine = EventDrivenEngine(
            ...
            max_sector_overweight=args.max_sector_overweight,
            trailing_stop=args.trailing_stop,
            weight_by_score=not args.no_score_weight,
